...
每个 Ticket 的子代理必须命名为 `Issue Task - {ticket_title}`，并显式使用与主代理相同的模型。Adapter 要求它加载 `implement` skill、只在记录的 feature worktree 中编辑、测试和提交，且不得创建分支、worktree 或 PR；它也不得编辑源 Issue 文件。
