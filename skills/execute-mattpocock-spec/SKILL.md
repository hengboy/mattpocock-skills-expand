...
然后为该 MattPocock Spec 创建 Git 分支：`git checkout -b feat/{feature-slug}`。分支名使用 `feat/` 前缀加 `{feature-slug}`。如果分支已存在则切换过去。将 `branch` 字段写入 `state.json`。
...
- 实施开始前**必须创建 Git 分支**（`feat/{feature-slug}`），绝不直接在 main/master 分支上执行。
