...
3. 调用 `plan.mjs` 的 Tracker materializer。首个实现只支持 `local` Markdown；没有已注入 adapter 的 GitHub、GitLab 或其他 Tracker 必须返回明确 `blocked`，不得伪造读取结果。
   - 新建单 Ticket Plan 可由调用方显式传入 `executionMode: "coordinator"`；仅在改动小、低风险、验证方式明确且不需要并行时选择它。默认是 `delegated`，多 Ticket Plan 不能选择 `coordinator`。
...
2. `execution_mode: delegated` 时调用 `completion-adapter.mjs` 的 Adapter：它必须并行派发该 Frontier 的所有 Ticket，并原生等待、收集所有终态；一个 blocked 结果不能提前结束收集。`execution_mode: coordinator` 时，主协调器通过注入的 `directExecutor({ ticket, worktree, plan })` 在同一 worktree 实施唯一 Ticket，并返回一份 Completion Result。
...
2. 用 `findMainWorktree` 找到 `main`；它只能包含本次执行的 Plan、Checkpoint 和 Issue 复选框改动。
...
- 多 Ticket Plan 必须使用 `delegated`；`coordinator` 仅适用于一个明确标记为小且低风险的单 Ticket Plan。
