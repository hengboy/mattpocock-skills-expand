...
- **Checkpoint** 是 feature worktree 中的 `.scratch/<feature>/state.json`——记录执行进度的唯一真实来源，并随 feature 分支提交。在 compaction 或中断后，信任它而非你的记忆。
...
Checkpoint 位于 feature worktree 的 `.scratch/<feature>/state.json`。若当前还未定位到 feature worktree，先检查主工作树中的旧版 checkpoint；若不存在，则按 `feat/{feature-slug}` 查找已注册 worktree 或分支中的该文件。找到后读取它。
...
确认新建或复用的 feature worktree 干净后，将 `branch`、绝对路径 `worktree` 和 `integration: { "status": "pending", "target_branch": "main" }` 写入 `<worktree>/.scratch/<feature>/state.json`。对于从主工作树发现的旧版 checkpoint，将它迁入此路径；此后 feature worktree 内的版本是唯一真实来源。

从此以后，所有会读取或改变实施结果的 Git 命令和所有 checkpoint 读写都必须在 feature worktree 中执行：`git -C <worktree> …`。每次写入并验证 checkpoint 后，必须在 feature 分支创建 checkpoint commit；普通 checkpoint 只暂存 `state.json`，而完成 Ticket 时按步骤 5 同时暂存该 Ticket 的 Issue 文件。不得把它留在主工作树或作为未提交改动。

然后判断 MattPocock Spec 是否已拆分（步骤 3），构建执行计划（步骤 4），并在任何分派之前写入并提交初始的 `state.json`。
...
1. 将该层内所有 `pending` Ticket 标记为 `in_progress`，记录各自的 `start_commit` 和 `started_at`，追加 `history` 条目，写入并提交只包含 `state.json` 的 dispatch checkpoint。
...
3. 按 Ticket 逐个处理返回的每个 Completion Result；一个 Ticket 的处理与 checkpoint commit 必须完成后，才能处理下一个：
   - **`done`**（已验证的完成结果）-> 先验证 Completion Result 的每个代码 commit 都存在于 feature 分支，取其最后一个作为 implementation commit。对本地 Markdown Tracker，在 `<worktree>/.scratch/<feature>/issues/<NN>-<slug>.md` 中核对验收项已满足，再将其中所有 `- [ ]` 复选框改为 `- [x]`。只有该文件已更新且不再包含未勾选复选框后，才能标记该 Ticket 为 `done`，将 `end_commit` 记录 Completion Result 中最后一个代码 commit 的 SHA，并记录 `completed_at`。未拆分的本地 Spec 同样处理其源 Markdown 文件；没有复选框的文件无需修改。若无法更新 Issue 文件或仍有未勾选项，则将 Ticket 标记为 `blocked` 并停止流程。真实 Tracker 不存在本地 Issue 文件时不适用此步骤。
...
4. 每个 `done` Ticket 的代码提交后，主代理必须在该代码提交之后立即创建一个 checkpoint commit：在 `<worktree>` 写入并验证 `state.json`、追加对应 `history`，并暂存该 `state.json` 与已勾选的本地 Issue 文件。**对含复选框的本地 Issue，同一个 checkpoint commit 必须同时包含已勾选的 Issue 文件和 `state.json`，且不得混入其他文件。** 没有复选框或不在本地的 Ticket 无需改动 Issue 文件，但仍必须提交 `state.json`。提交后确认 worktree 干净；这个 checkpoint commit 记录元数据，`end_commit` 仍指向前一个 implementation commit，绝不写入 checkpoint commit 自身的 SHA。

5. 该层全部处理完后，确认每个结果都已有其 checkpoint commit；如果该层有 `blocked` 结果则停止；否则进入下一层级。

**连续协调约束：** 完成一个 Frontier 并告知用户进度，不是本次 Spec 执行的终点。进度通知只能作为 `commentary` 中间更新，不得以 `final response`、结束当前执行或等待用户继续来结束一个 Frontier。写入成功的 checkpoint 后，主代理必须在同一协调会话中立即继续派发下一个可执行 Frontier，直至出现 `blocked` 结果、需要步骤 6b 的用户确认，或整个 Spec 已完成。不得通过重新启动一个独立的 `codex` 执行来推进下一层。

在派发下一 Frontier 前，必须确认 Completion Adapter 已原生等待并收集上一 Frontier **全部**子代理的终态，使这些子代理会话不再占用当前协调器的派发容量。若 Codex 在派发下一层的某个 Ticket 时报告会话/容量冲突，先完成现有的原生 `wait/collect` 清理，再在同一协调会话中重试该次派发一次；不得先向用户结束执行。重试仍失败时，Adapter 必须为未成功派发的 Ticket 返回 `blocked` Completion Result，编排器据此记录错误并停止。不得把已经标记为 `in_progress` 的 Ticket 静默退回 `pending`，也不得为了恢复派发而改变 feature 分支或 worktree。
...
- `done` Ticket 必须记录 `end_commit` 与 `completed_at`；`end_commit` 是 Completion Result 最后一个 implementation commit，不能是随后写入 Issue/checkpoint 元数据的 checkpoint commit。
...
- 每个已完成的 Ticket 都必须紧随代码提交创建 checkpoint commit；对含复选框的本地 Ticket，该提交只包含其已勾选 Issue 文件和 `state.json`。不得把这些元数据留在主工作树、留为未提交改动，或合并到无关 Ticket 的提交中。
