...
description: "全自动执行 MattPocock Spec。如果 /to-tickets 已将其拆分为多个 Ticket，按依赖顺序逐个委派子代理（每个运行 /implement）；否则在当前上下文中运行 /implement。最后对整个 MattPocock Spec 做 /code-review。通过 state.json 检查点断点续传，永不从头开始。"
disable-model-invocation: true
---

# Execute MattPocock Spec

将一个 MattPocock Spec 从签署完成的计划推进到经过评审、已提交的代码——并能从中断中恢复。根据 `/to-tickets` 是否已将 MattPocock Spec 拆分为多个 Ticket 进行分支：已拆分时沿着 **frontier（前沿）** 逐个委派子代理执行 Ticket；未拆分时在当前上下文中运行 `/implement`。无论哪种方式，最后都对整个 MattPocock Spec 做 `/code-review`。`state.json` 文件记录全生命周期状态，使得重新运行时从最后一个检查点 **恢复**，而不会重新开始。
...
- `status: "executing"`：
  - 对于 `mode: "single"` -> 如果单个 Ticket 是 `done`，转到步骤 6；否则重新分派它（步骤 5，单分支）。
  - 对于 `mode: "multi"` -> 从最低的未完成层级开始恢复：找到第一个包含非 `done` Ticket 的层级，将该层中所有 `pending` 和 `in_progress` 的 Ticket 作为当前层级，按步骤 5 的多分支方式重新分派该层（并行）。已 `done` 的 Ticket 跳过——它们的 commit 已经落地；永不重新分派它们。
...
如果是新初始化，立即写入 `state.json`（来自步骤 3-4）。然后分派。**每次**子代理返回后，优先更新 `state.json`——checkpoint 必须能够承受下一次中断。

#### 单分支（未拆分）-> 在当前上下文中执行 /implement

加载 `/implement` skill 并针对 MattPocock Spec 运行。`/implement` 驱动 `/tdd` 在 MattPocock Spec 预约定的 seam 处进行，定期运行 typecheck 和测试，最后对其 diff 运行自己的 `/code-review`——该评审**就是**未拆分 MattPocock Spec 的最终评审。按照 `/implement` 的指示提交到当前分支。

将单个 Ticket 标记为 `done`，记录其 `end_commit`（`git rev-parse HEAD`），设置 `status: "complete"`，并追加一条 `history` 条目。跳到步骤 7。

#### 多分支（已拆分）-> 按 frontier 层级逐层分派

**逐层推进，层内并行，层间串行。** 对于每个 frontier 层级：
...
- 在当前层级所有子代理返回之前，绝不进入下一层级——跨层级有 blocking 依赖。
